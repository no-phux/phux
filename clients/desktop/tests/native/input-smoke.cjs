...
let sequence = 0;
const command = (kind, text = '', extra = {}) => batch([['setCustomProp', 2, 'command', {kind, text, sequence: ++sequence, ...extra}]]);
async function until(predicate, message) {
  const deadline = Date.now() + 15000;
  while (!predicate()) {
    assert.ok(Date.now() < deadline, `${message}; snapshot=${host.inputFixtureSnapshot(2)}; bytes=${JSON.stringify(bytes())}; events=${JSON.stringify(events.slice(-8))}; runtimeError=${client.lastError()}`);
    await new Promise(resolve => setTimeout(resolve, 10));
    renderer.flush();
  }
}
async function main() {
  client.connect({socketPath, cols: 80, rows: 24}, () => { if (!closed) events.push(...client.takeEvents()); });
  ...
  await until(() => events.some(e => e.kind === 'AttachAnswered' && e.terminalId === pane.terminalId), 'initial terminal subscription confirmed');
  ...
  batch([
    ['createElement', 1, 'div'], ['setStyle', 1, {width:800,height:480}], ['setRoot', 1],
    ['createElement', 2, 'phux-input-fixture'], ['setStyle', 2, {width:800,height:480}],
    ['setCustomProp', 2, 'handle', client.handle], ['setCustomProp', 2, 'view', view],
    ['setCustomProp', 2, 'terminal', terminal], ['appendChild', 1, 2],
  ]);
  ...
  await until(() => /\x1b\[<0;\d+;\d+M\x1b\[<0;\d+;\d+m$/.test(bytes()), 'mouse protocol press/release');
  const beforeSelection = bytes();
  renderer.simulateMouseDown(5, 5, 0, 'shift'); renderer.flush();
  renderer.simulateMouseMove(100, 5, 0, 'shift'); renderer.flush();
  renderer.simulateMouseUp(100, 5, 0, 'shift'); renderer.flush();
  ...
  renderer.simulateScrollWheel(25, 25, 0, 20, 'shift'); renderer.flush();
  ...
  renderer.simulateKeyDown('c->c'); command('commit', 'c');
  renderer.simulateKeyDown('c->c', true); command('commit', 'c');
  ...
  await until(() => events.some(e => e.kind === 'InputDelivery' && e.outcome === 'Delivered'), 'sole owner delivery receipt');
  command('paste', 'unsafe\n');
  await until(() => events.some(e => e.kind === 'InputDelivery' && e.outcome === 'Refused'), 'paste safety refusal');
  ...
  await until(() => viewState(view) && viewState(view).stream !== oldStream, 'new replica identity');
  ...
  console.log(JSON.stringify({result:'pass', evidence:'GPU key/mouse dispatch, IME callbacks and UTF16 graphemes, PTY exact bytes, Kitty repeat/release, application mouse versus Shift view-local selection/scroll, acknowledged paste/refusal, inactive/resync/destroyed/closed fences', bytes: bytes(), events: events.filter(e => e.kind === 'InputDelivery')}));
}
