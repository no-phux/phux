...
let mounted = false;
let views = [];
const batch = (ops) => { renderer.applyBatch(JSON.stringify(ops)); renderer.flush(); };
function repaint() {
  if (mounted) batch([2, 3].map(id => ["setCustomProp", id, "paintRevision", ++revision]));
}
function report(id) { return host.terminalFixturePaints().find(report => report.id === id); }
function text(id) { return report(id)?.glyphs.map(glyph => glyph.text).join("") ?? ""; }
async function until(predicate, name) {
  const deadline = Date.now() + 15000;
  while (!predicate()) {
    if (Date.now() >= deadline) {
      console.error("publication generations", views.map(view => host.terminalFixtureGeneration(client.handle, view)));
      writeFileSync(resolve(artifact, "failure.json"), JSON.stringify(host.terminalFixturePaints(), null, 2));
      renderer.captureScreenshot(resolve(artifact, "failure.png"));
    }
    assert.ok(Date.now() < deadline, `timeout: ${name}; ${JSON.stringify(host.terminalFixturePaints()).slice(0, 500)}`);
...
async function rasterFixtures() {
  ...
  writeFileSync(resolve(artifact, "pixel-geometry.json"), JSON.stringify({ cellWidth: geometry.cellWidth, cellHeight: geometry.cellHeight, scale: geometry.scale }));
  capture("block");
  batch([["setCustomProp", 2, "focused", false]]);
  capture("hollow");
  batch([["setCustomProp", 2, "focused", true], ["setCustomProp", 2, "cursorVisible", false]]);
  capture("hidden");
  batch([["setCustomProp", 2, "cursorVisible", true]]);
  for (const [command, name] of [["CURSORBAR", "bar"], ["CURSORUNDER", "underline"]]) {
  ...
  batch([["setStyle", 2, { width: clipWidth, height: clipHeight }], ["setStyle", 3, { width: 0, height: 0 }]]);
  capture("clipped");
  batch([["setStyle", 2, { width: 600, height: 320 }], ["setStyle", 3, { width: 600, height: 320 }]]);
  await send("DRAW");
  await until(() => text(2).startsWith("GPUI NATIVE TERMINAL") && text(2).includes("READY"), "restore dense fixture");
}
let terminal;
try {
  ...
  await until(() => client.topology()?.sessions.some(session => session.name === "terminal-painter"), "topology");
  client.attachSession("terminal-painter");
  await until(() => client.status() === "Attached", "attach");
  terminal = client.topology().panes.find(pane => pane.sessionName === "terminal-painter").terminalId;
  ...
  const ops = [["createElement", 1, "div"], ["setStyle", 1, { width: 1200, height: 340, display: "flex", flexDirection: "row", backgroundColor: "#fa00fa" }], ["setRoot", 1]];
  for (const [id, view] of [[2, left], [3, right]]) ops.push(
    ["createElement", id, "phux-terminal"], ["setStyle", id, { width: 600, height: 320 }],
    ["setCustomProp", id, "clientHandle", client.handle], ["setCustomProp", id, "terminalId", terminal],
    ["setCustomProp", id, "viewId", view], ["appendChild", 1, id]);
  batch(ops);
  mounted = true;
  await send("DRAW");
  await until(() => text(2).includes("READY") && text(3).includes("READY"), "paint actual PTY output");
  assert.deepEqual(renderer.getPaintedText(), [], "terminal does not synthesize GPUIX variable-width text rows");
  ...
  assert.deepEqual(unicode.map(({ text, col }) => [text, col]), [["A", 0], ["界", 1], ["é", 3], ["🙂", 4], ["Z", 6]]);
  for (const glyph of unicode) {
    assert.equal(glyph.paintX, unicode[0].x + glyph.col * initial.cellWidth);
    assert.ok(Math.abs(glyph.baseline - glyph.y - initial.baselineOffset) < 0.001, "fallback and combining glyphs share the fixed row baseline");
  }
  ...
  writeFileSync(resolve(artifact, "initial.json"), JSON.stringify(host.terminalFixturePaints(), null, 2));

  const timings = [];
  for (let frame = 0; frame < 30; frame++) {
    const started = performance.now();
    repaint();
    timings.push({ flushMillis: performance.now() - started, prepareMicros: report(2).prepareMicros, paintMicros: report(2).paintMicros });
  }
  ...
  batch([["setCustomProp", 3, "font", { family: "Menlo", size: 18, lineHeight: 1.4 }], ["setCustomProp", 3, "theme", { background: "#152030", foreground: "#b0d0f0" }], ["setCustomProp", 3, "focused", false]]);
  ...
  batch([
    ["createElement", 2, "phux-terminal"], ["setStyle", 2, { width: 600, height: 320 }],
    ["setCustomProp", 2, "clientHandle", client.handle], ["setCustomProp", 2, "terminalId", terminal],
    ["setCustomProp", 2, "viewId", replacement], ["appendChild", 1, 2],
  ]);
  ...
  console.log(JSON.stringify({ result: "pass", artifact, scale: initial.scale, glyphs: initial.glyphs.length, prepareMicros: initial.prepareMicros, paintMicros: initial.paintMicros }));
} finally {
