...
// Native SDK v0.10.0 added updater callbacks that are exported by the app's
// Zig runtime. This standalone measurement links the AppKit host directly and
// never exercises the updater, so satisfy those callbacks with the same
// refusal stubs used by the SDK's host-only test harness.
...
int native_sdk_update_verify_archive(
    const char *path, size_t path_len, uint64_t expected_bytes, const char *sha256, size_t sha256_len) {
