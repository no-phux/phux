...
export function enqueueOperationCommand(state: TabCommandState, operation: Uint8Array): TabCommandDecision {
  if (operation.length !== 12 || operation[0] !== 1) return { state: { ...state, outcome: 3 }, request: EMPTY };
