...
pub const Request = struct { id: u64, target: Target };

pub fn decode(bytes: []const u8) ?Request {
    if (bytes.len != request_len) return null;
    if (bytes[0] != 1 or bytes[1] != 1 or bytes[10] != 1) return null;
    if (bytes[11] >= model_module.max_windows) return null;
...
pub const Receipt = struct {
    ...
    pub fn encode(self: Receipt) [receipt_len]u8 {
        ...
        out[0] = 1;
        out[1] = if (self.reason == .none) 1 else 2;
