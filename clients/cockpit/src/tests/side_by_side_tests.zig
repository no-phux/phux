...
//! (`Model.phux_peer`), listed as host groups in one switcher. This Mac's
...
test "the switcher lists this Mac's sessions first, then the remote host's, whichever is active" {
    ...
    try testing.expectEqual(@as(u32, 1), first[1].peer_session);
    try testing.expectEqual(@as(u32, 2), first[2].peer_session);
    ...
    try testing.expectEqual(@as(u32, 1), second[0].peer_session);
    try testing.expectEqual(@as(u32, 2), second[1].peer_session);
...
const RestartCounter = struct {
    ...
    pub fn restartPeer(self: *@This(), _: *ts_engine.Engine) bool {
        self.peer += 1;
        return true;
    }
};

test "selecting a standby session exchanges the two, and the old host's rows stop listing at once" {
    if (comptime !support.phux_enabled) return error.SkipZigTest;
    var pair = try Pair.start(false);
    defer pair.engine.destroy();
    const model = pair.engine.model;

    const target = targets.capture(model, .{ .peer_session = 2 }).?;
    var bytes: [targets.max_len]u8 = undefined;
    const decoded = targets.decode(target.encode(&bytes)).?;
    try testing.expectEqual(@as(u32, 2), decoded.resolve(model).?.peer_session);
    // The active coordinator's session 2 does not exist; the standby's does.
    try testing.expect(targets.capture(model, .{ .session = 2 }).?.resolve(model) == null);

    var fx: RestartCounter = .{};
    try testing.expect(pair.engine.switchToPeer(2, &fx));
    ...
    // (b) The standby still held mini's sessions while its group is already
    // labelled This Mac: they must not list, and a held row must not attach
    // `deploy` by name on the wrong host.
    ...
    // Removing the standby withdraws its group entirely.
    pair.engine.dropPeer(&fx);
    try testing.expect(model.phux_peer == null);
    try testing.expect(decoded.resolve(model) == null);
}

test "a standby that disconnects stops listing and stops accepting its old sessions" {
    if (comptime !support.phux_enabled) return error.SkipZigTest;
    var pair = try Pair.start(false);
    defer pair.engine.destroy();
    const model = pair.engine.model;
    try testing.expectEqual(@as(usize, 2), peerEntries(model));
    const held = targets.capture(model, .{ .peer_session = 1 }).?;
    try testing.expect(held.resolve(model) != null);

    // (a) The standby's connection fails: its group empties, and a row held
    // from before cannot be activated.
    pair.peer.stop();
    try testing.expectEqual(@as(usize, 0), peerEntries(model));
    try testing.expectEqual(@as(usize, 0), pair.peer.host.sessionCatalog().len);
    try testing.expect(held.resolve(model) == null);
    var fx: RestartCounter = .{};
    try testing.expect(!pair.engine.switchToPeer(1, &fx));
...
test "Connect to Host keeps this Mac beside the host; Use this Mac keeps the host; Disconnect removes it" {
    ...
    try testing.expect(engine.model.phux_peer == null);
    ...
    const peer = engine.model.phux_peer.?;
    ...
    try testing.expect(engine.model.phux_peer == peer);
    ...
    // Disconnect: only the host's group goes.
