...
function outcomeNotice(outcome: number): Uint8Array {
  if (outcome === 3) return asciiBytes("Could not save. Your preview is still active. Retry or cancel to restore.");
...
function validFlags(bytes: Uint8Array): boolean {
  ...
  return bytes[2] <= 5 && bytes[5] <= 2 && bytes[6] <= 1;
}

function validResponse(bytes: Uint8Array): boolean {
  if (bytes.length < 10 || bytes[0] !== 1) return false;
  if (!validFlags(bytes)) return false;
  return 10 + bytes[8] + bytes[9] === bytes.length;
}
...
export function appearanceResponse(bytes: Uint8Array): Appearance | null {
  ...
  const rawOutcome = bytes[2];
  const outcome = rawOutcome >= 0 && rawOutcome <= 5 ? Math.trunc(rawOutcome) : 5;
  const rawTheme = bytes[4];
  const rawCursor = bytes[5];
  const rawPlacement = bytes[6];
  const theme = rawTheme >= 0 && rawTheme <= 255 ? Math.trunc(rawTheme) : 255;
  const cursor = rawCursor >= 0 && rawCursor <= 2 ? Math.trunc(rawCursor) : 0;
  const placement = rawPlacement >= 0 && rawPlacement <= 1 ? Math.trunc(rawPlacement) : 0;
  return { theme, cursor, placement, active: bytes[1] === 1, outcome, dirty: bytes[3] === 1,
    overrides: bytes[7] === 1,
    fontLabel: bytes.slice(10, fontEnd), contrastLabel: bytes.slice(fontEnd), notice: outcomeNotice(bytes[2]) };
}
