//! Schemas for the orchestration tools, and the two that still run the CLI.
//!
//! `phux_launch` and `phux_workspace` stay on the CLI residue (their reasons
//! are in `crate::tool_table`); spawn, signal, tag, rename, and the spatial
//! edits run in-process in `crate::pane_tools`, dispatched from here.
...
pub(crate) async fn call(name: &str, args: &Value) -> Result<Value, ToolError> {
    use crate::pane_tools::{self, Spatial};
    match name {
        "phux_spawn" => pane_tools::spawn(args).await,
        "phux_signal" => pane_tools::signal(args).await,
        "phux_tag" => pane_tools::tag(args).await,
        "phux_rename" => pane_tools::rename(args).await,
        "phux_insert_pane" => pane_tools::spatial(args, Spatial::Insert).await,
        "phux_move_pane" => pane_tools::spatial(args, Spatial::Move).await,
        "phux_swap_pane" => pane_tools::spatial(args, Spatial::Swap).await,
...
"#,
    ...
    #[tokio::test]
    async fn dispatch_validation_blocks_malformed_residue_calls_before_execution() {
        let adapter = CliAdapter::new("must-not-execute");
        for (name, args) in [
            ("phux_launch", json!({})),
            (
                "phux_launch",
                json!({ "integration": "codex", "list": true }),
            ),
            (
                "phux_launch",
                json!({ "integration": "codex", "split": "vertical" }),
            ),
            ("phux_workspace", json!({ "action": "delete" })),
            (
                "phux_workspace",
                json!({ "action": "restore", "archive": "-" }),
            ),
        ] {
            assert!(
