...
/** Physical-key values are stable and match the phux wire/libghostty atoms. */
...
/** Opaque, owning-thread-only terminal projection. Never Send/Sync. */
...
typedef struct PhuxClientOptions {
    size_t size;
    uint32_t version;
    /** ABI-v1 compatibility fields. Ignored by the protocol-0.6 projection. */
...
typedef struct PhuxAttachOptions {
    size_t size;
    uint32_t version;
    /** ABI-v1 compatibility field. Protocol 0.6 does not carry attach IDs. */
    uint32_t attach_id;
    ...
    uint16_t cols;
    uint16_t rows;
    uint8_t has_pixel_size;
    uint16_t pixel_width;
    uint16_t pixel_height;
    uint8_t request_scrollback;
...
/** Reserved ABI-v1 values; the protocol-0.6 projection emits no jobs. */
...
typedef enum PhuxClientDamageKind {
    ...
    /** Replica exists, but synchronized-output mode defers publishable damage. */
    PHUX_CLIENT_DAMAGE_PRESENT = 4
...
typedef enum PhuxClientStatusKind {
    ...
    /** Reserved ABI-v1 values; protocol 0.6 has no paged-history frames. */
...
/**
 * Borrowed effect. bytes contains title/error detail when defined by kind.
 * stream_id is the bridge replica generation for terminal-scoped effects;
 * bootstrap_id remains reserved and zero in ABI v1.
 */
 ...
 * slices address the separate utf8 arena. stream_id is the bridge replica
 * generation. bootstrap_id, paged history fields, and top_anchor are reserved
 * ABI-v1 fields; the current protocol-0.6 projection returns zero/false values
 * for them. The scrollbar total, offset, and visible-row fields remain live.
 */
...
typedef struct PhuxKeyEvent {
    ...
    uint8_t has_text;
    PhuxBytes text;
    uint8_t has_unshifted_codepoint;
...
/** Runs owning-thread time-based maintenance, including synchronized-output expiry. */
PhuxClientResult phux_client_maintenance(PhuxClient *client);
/** True while the owning thread must continue scheduling maintenance calls. */
bool phux_client_maintenance_pending(const PhuxClient *client);
...
PhuxClientResult phux_client_terminal_resize_with_pixels(PhuxClient *client, const PhuxTerminalId *terminal_id, uint16_t cols, uint16_t rows, uint8_t has_pixel_size, uint16_t pixel_width, uint16_t pixel_height);
PhuxClientResult phux_client_viewport_resize(PhuxClient *client, uint16_t cols, uint16_t rows, uint8_t has_pixel_size, uint16_t pixel_width, uint16_t pixel_height);
