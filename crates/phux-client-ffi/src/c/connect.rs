...
use crate::c::error::{BridgeError, check_struct};
use crate::c::remote::text_in;
use crate::c::remote::{PhuxRemoteTunnel, config_path_in};
use crate::c::types::{PhuxBytes, PhuxClientOptions, PhuxClientResult};
use crate::c::{PhuxClient, client_limits, guard, with_client_mut};
...
pub unsafe extern "C" fn phux_client_poll(client: *mut PhuxClient) -> PhuxClientResult {
    ...
    let result = with_client_mut(client, |client| {
        ...
        notify_attached = crate::c::poll_connected(client)?;
        Ok(())
    });
    if result == PhuxClientResult::Ok && notify_attached {
        crate::c::invoke_attached(client)
    } else {
        result
    }
}
...
pub unsafe extern "C" fn phux_client_connection_error(
    ...
    with_client_mut(client, |client| {
        ...
        *out = crate::c::types::bytes_out(&client.connection_error);
        Ok(())
    })
}
