...
//!
//! MODIFIED FROM UPSTREAM (`xai-crash-handler`): identifier renames only
//! (`grok_version` -> `app_version`). No logic changes — see the crate NOTICE.
//!
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("phux-crash-test-replace");
    ...
    fn install_creates_owner_only_crash_blob() {
        ...
        let _guard = SIGNAL_STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = std::env::temp_dir().join(format!("phux-crash-test-0600-{}", std::process::id()));
    ...
    fn install_tightens_preexisting_0644_crash_blob() {
        ...
        let _guard = SIGNAL_STATE_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir =
            std::env::temp_dir().join(format!("phux-crash-test-tighten-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("create crash dir");
