...
//!
//! VENDORED from xAI's `xai-crash-handler` (Apache-2.0) — see this crate's
//! NOTICE and the Cargo.toml header for the full modification list. This file
//! is MODIFIED FROM UPSTREAM: renamed identifiers, a `Debug` derive on
//! [`CrashHandlerConfig`], and phux-targeted doc comments. No logic changes.
//!
...
//!
//! if let Some(report) = phux_crash::check_previous_crash(&crash_dir) {
...
//! }
//!
//! phux_crash::install(phux_crash::CrashHandlerConfig {
...
/// phux's state directory they must not be world-readable.
...
mod tests {
    ...
    fn check_previous_crash_returns_none_when_no_file() {
        let dir = PathBuf::from("/tmp/phux-crash-test-nonexistent");
