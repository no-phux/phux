...
# buys no depth. history-bytes is the one that decides how deep a pane is:
#
...
#   32 MiB          ~47500               ~19031
#
# Attach does not scale with this number: the native bootstrap takes a lease
# on the retained history and encodes nothing up front; each page is encoded
# from the live scrollback only when a client asks for it. What it costs is
# resident memory, per pane, for the life of the session -- so multiply by
# your pane count before raising it. 67108864 (64 MiB) is the accepted
# maximum. Pruning is page-granular, so the engine keeps at least one
# standard page of history whatever you set.
history-limit = 50000
history-bytes = 2097152
# Bytes of AgentEventsJsonlV1 records an agent session retains (ADR-0103).
# The ring is replayed when a client attaches to the session stream, so this
# is both how far back `phux agent log` can see and what each attach re-sends.
# An append past the ceiling evicts the oldest records and counts a tombstone
...
# name stays selector-safe); ${random-name} expands to a generated
# adjective-noun pair such as drifting-cedar (`phux new` redraws a taken
# pick, then falls back to a numeric suffix); unknown placeholders pass
# through verbatim.
#
...
#
# The active tab is the one lime chip (accent on the selected surface).
# Inactive tabs are dim text with no fill, so the bar is not a slab.
# Session name and clock recede to the same dim. The chip clears 4.5:1
# against the selected surface; dim text against a typical dark terminal,
# the contrast floor documented in crates/phux-tui/src/render/theme.rs.
...
left = [
  { kind = "windows", format = " {index}:{name} ", separator = " ", active = { fg = "#bef264", bg = "#293628", bold = true }, inactive = { fg = "#9aa4b2" } },
]
center = []
...
# in code (crates/phux-tui/src/render/theme.rs); the block is commented
...
# The sidebar (prefix-b toggles it): two fixed panels, ON by default.
#
#   Agents      cross-session agents in stable session/window/pane order.
#               State changes update badges without reordering rows.
#   Sessions    session names and serving hosts, with the current session's
#               windows nested below. Each panel keeps half the body height.
#
