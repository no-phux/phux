...
| `agent log` (resource-model branch) | `0` the retained records were printed, or a `--follow` ended by Ctrl-C or by the session closing; `2` `wrong_resource_kind` or `unsupported_server`; `1` no server, selector miss, transport, or the server going away mid-follow — a session that has already closed is a selector miss (`no_such_target`, exit 1), not a refusal, because its records went with it. No `124`: there is no `--timeout`. |
