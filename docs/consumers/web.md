...
Use the native or Nix tools in the [setup guide](../SETUP.md#browser-client):
...
Use [Contributor setup](../SETUP.md#browser-client) for native or Nix tools.
Engine changes use `bash scripts/build-vt-wasm.sh`, which fetches and verifies
the standalone checkpoint engine's pinned source before building and ABI-testing
it. `--check` compares regenerated bytes to the committed artifact. Both need
the release-pinned Zig from `scripts/install-zig.sh` first on PATH; nixpkgs'
x86_64 Linux `zig_0_16` compiles one function differently and fails `--check`.
