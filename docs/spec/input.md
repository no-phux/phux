...
**TL;DR.** The L1 client-to-server input surface carries structured key,
mouse, focus, and paste events. Structured events let phux faithfully
transport the kitty keyboard protocol, IME composition, modifier-rich chords,
and pixel-precise mouse events. One narrow exception,
`INPUT_TERMINAL_REPLY`, carries byte-exact replies generated by the client's
terminal emulator; it is never user input or paste, and the server discards
it because its canonical terminal already answered.

---

## 1. Overview

This section specifies the L1 client-to-server input messages: their wire shape
and the server-side pipeline that writes them to the PTY. User input is
structured and terminal-mode-agnostic until the server encodes it. Terminal
query replies come from the server's canonical terminal; a client engine's
copy uses the separate opaque `INPUT_TERMINAL_REPLY` path and is discarded
(§6).
...
`INPUT_TERMINAL_REPLY = 0x17` is a client-to-server, field-tagged TLV frame:
...
`bytes` is a non-empty opaque byte string of at most 65,536 bytes. NUL, ESC,
non-UTF-8, and future terminal-protocol replies are valid and MUST remain
byte-identical. The codec and hosts MUST NOT decode it as text, an `InputEvent`,
or a `PasteEvent`; in particular, bracketed-paste mode MUST NOT add paste
delimiters or otherwise rewrite it.

The client emits this frame only for bytes generated by its terminal emulator
while applying server output (for example, a terminal query response), and
only when `HELLO_OK.server_caps.features` contains `TERMINAL_REPLY = 0x80`.
Absent that bit, including an older 0.7 HELLO_OK with no feature bits, the
client MUST NOT put the frame on the wire: it discards the generated reply,
surfaces a visible local unsupported-capability status, and MUST NOT reinterpret
the bytes as structured input or paste. The frame is stateful: it is forbidden
before HELLO and requires a live attachment and input-authorized subscription
to `terminal_id`. The server MUST NOT write the bytes to the PTY and MUST NOT
route them through the structured event encoder; it discards them without an
`ERROR`.

The server's canonical terminal is the one authority for terminal query
replies (DSR, DA, DECRQM, XTWINOPS, OSC 10/11, and the rest). It answers each
query as it parses the child's output, exactly once, whether zero or many
clients are attached, and before any client could. Every client replica parses
the same output and generates the same reply a round trip later, so forwarding
one would hand the child a second answer it never asked for, which an
application reading its terminal reads as typed input. The frame and the
`TERMINAL_REPLY` bit remain so a client built against the earlier text keeps
negotiating; such a client MAY keep sending the frame, and a new client
SHOULD NOT.

Empty or oversized `bytes` is malformed. Unknown top-level fields are skipped
by declared length; fields 1 and 2 remain required.

A server that did not advertise `TERMINAL_REPLY` MUST discard a received
`INPUT_TERMINAL_REPLY`, send an uncorrelated
`ERROR { code: UNKNOWN_MESSAGE_TYPE }`, and continue without writing any
payload byte to the PTY.
...
Attached `INPUT_*` frames, including `INPUT_TERMINAL_REPLY`, require an active
subscription to the target Terminal. The input lease ([ADR-0033]) then governs
every input surface: while held, only the lease holder's attached input,
`ROUTE_INPUT`, or `APPLY_INPUT` may reach the PTY. Other
attached/fire-and-forget input is dropped; `APPLY_INPUT` receives
`ERROR(INPUT_LEASE_HELD)` before handoff.

The four structured atoms (`KEY` / `MOUSE` / `FOCUS` / `PASTE`) can also be
delivered without an attach via `ROUTE_INPUT` or the acknowledged `APPLY_INPUT`
