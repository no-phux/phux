...
while applying server output (for example, a terminal query response). It is
stateful: it is forbidden before HELLO and requires a live attachment and
input-authorized subscription to `terminal_id`. The server enqueues the bytes
directly on that Terminal's ordered encoded-input lane. It never routes this
frame through the structured event encoder.
