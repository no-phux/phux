...
terminal emulator; it is never user input or paste.
...
structured and terminal-mode-agnostic until the server encodes it. Emulator
replies are already encoded by the client engine and use the separate opaque
`INPUT_TERMINAL_REPLY` path.
...
only when `HELLO_OK.server_caps.features` contains `TERMINAL_REPLY = 0x40`.
...
to `terminal_id`. The server enqueues the bytes directly on that Terminal's
ordered encoded-input lane. It never routes this frame through the structured
event encoder.
