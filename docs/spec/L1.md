...
> **Status: spec-only.** The reference codec carries no `kind` field and the
> reference server serves Terminal-kind resources only, so every id on the
...
<!-- impl-status: spec-only; probe: CloseReason,ParentNotFound,ParentKindMismatch -->
> **Status: spec-only.** `SPAWN_TERMINAL` fields 11 through 14,
> `TERMINAL_CLOSED` field 3, and the three `SpawnError` variants below have no
> codec entry, and the reference server keeps no binding graph. A Terminal
> today has no parent and no children.
...
> **Status: spec-only.** `BootstrapCodec` has no such tag, no server keeps a
> record ring, and nothing appends to a stream except a PTY. The codec is
> the output grammar of the `AGENT_SESSION` kind (§1.1), and lands with it.
...
<!-- impl-status: spec-only; probe: COMMAND_TAG_APPEND_RESOURCE_OUTPUT,NOT_PRODUCER,RECORD_INVALID -->
> **Status: spec-only.** Tag `0x1a` has no codec entry and decodes as
> `UnknownEnumValue`; the three error codes named below are not in the
> shipped `ErrorCode` enum. The command is the input channel of the
> `AGENT_SESSION` kind (§1.1) and lands with it.
...
<!-- impl-status: spec-only; probe: AgentEventsJsonlV1 -->
> **Status: spec-only.** The reference detector ranks hook, process, and
> screen evidence ([ADR-0046](../../ADR/0046-server-side-agent-state-detection.md),
> [ADR-0085](../../ADR/0085-hook-sourced-agent-state.md)); there is no stream
> to rank above them until the `AGENT_SESSION` kind lands.
