...
> **Status: partial.** The reference codec carries `kind` and the
> `RESOURCE_KINDS` bit, but the reference server serves Terminal-kind
> resources only and does not yet advertise the bit, so every id on the
> wire today names a Terminal and a consumer written against this section
> behaves identically against the current server. The section states the
> contract a server advertising `RESOURCE_KINDS` satisfies.
...
the `TerminalId` / `TERMINAL_*` names throughout this document are that
...
NOT reallocate a retired tag. The kind is fixed by `SPAWN_TERMINAL` field 11
(§1.2) and never changes for the life of the resource.

**Facets.** Every kind shares the substrate: identity; spawn and close
(§3.1); the `ATTACH_TERMINAL` / `DETACH_TERMINAL` output subscription with
its §4 generations; `KILL_TERMINAL` and `KILL_TERMINALS`;
`SUBSCRIBE_TERMINAL_EVENTS` and `SUBSCRIBE_EVENTS`; `GET_STATE`; and the L3
...
|---|---|---|
| `TERMINAL` | `cols`, `rows`, `title`, `cwd`, PTY child | `INPUT_KEY`, `INPUT_PASTE`, `INPUT_MOUSE`, `INPUT_FOCUS`, `INPUT_RAW`, `INPUT_TERMINAL_REPLY`, `TERMINAL_RESIZE`, `HISTORY_REQUEST`, `FRAME_ACK`, `MOVE_TERMINAL`; commands `GET_SCREEN`, `ROUTE_INPUT`, `APPLY_INPUT`, `PUT_FILE`, `TRANSCRIBE`, `GET_TERMINAL_STATE`, `ACQUIRE_INPUT`, `RELEASE_INPUT`, `SIGNAL_TERMINAL`, `REPORT_ASKED`, `REPORT_AGENT_STATE` |
...
(`INPUT_*`, `TERMINAL_RESIZE`, `HISTORY_REQUEST`, `FRAME_ACK`) the server
...
**Unknown-kind tolerance.** `TerminalInfo` gains three trailing additive
...
TerminalInfo {
    ...
    parent: optional<TerminalId>,     // trailing; §1.2
...
Terminal-facet frame, and SHOULD NOT `ATTACH_TERMINAL` to it, since the
stream's codec (§4.3) is defined by the kind. `KILL_TERMINAL`, event
...
through an explicit `ATTACH_TERMINAL`.

A client MUST NOT send `SPAWN_TERMINAL` field 11, a non-zero kind, or
...
<!-- impl-status: partial; probe: CloseReason,ParentNotFound,ParentKindMismatch -->
> **Status: partial.** `SPAWN_TERMINAL` fields 11 through 14,
> `TERMINAL_CLOSED` field 3, and the three `SpawnError` variants below
> round-trip through the reference codec, but the reference server keeps no
> binding graph and states no close reason. A Terminal today has no parent
> and no children.
...
is one level deep, and closing the parent closes the child. `SPAWN_TERMINAL`
...
SPAWN_TERMINAL {
    ...
    parent: optional<TerminalId>,     // field 12; the Terminal-kind resource this one binds to
...
re-parents a resource; `MOVE_TERMINAL` moves a Terminal between windows and
...
`KILL_TERMINAL`, membership in a `KILL_TERMINALS` batch, or server shutdown —
...
identical to `KILL_TERMINALS` (§5.2): no observer sees a state in which the
parent is gone and a child remains, or a `GET_STATE` snapshot listing a child
whose parent is absent. Each child receives its own `TERMINAL_CLOSED` with
`reason = PARENT_CLOSED`; the per-resource frames follow the commit
asynchronously, as they do after `KILL_TERMINALS`, and a consumer MUST NOT
infer an order between a parent's frame and its children's. No frame
carrying a child's id is emitted after that child's `TERMINAL_CLOSED`.
Closing a child never affects its parent. A `KILL_TERMINALS` batch that
...
**`CloseReason`.** `TERMINAL_CLOSED` gains one additive field:
...
CloseReason = enum (u8) {
    ...
    KILLED          = 1,   // KILL_TERMINAL or KILL_TERMINALS named it
...
receiving `SPAWN_TERMINAL { satellite: Some(H), parent: Some(p), .. }`
...
`TerminalInfo`, and re-tags a child's `TERMINAL_CLOSED { reason: PARENT_CLOSED }`
...
SPAWN_TERMINAL   { request_id: u32,
                   ...
                   parent: optional<TerminalId>,      // field 12 (§1.2)
...
> **Status: partial.** `BootstrapCodec` carries the tag, but no server keeps
> a record ring and nothing appends to a stream except a PTY. The codec is
> the output grammar of the `AGENT_SESSION` kind (§1.1), and is served with it.
...
`BOOTSTRAP_BEGIN` / `CHUNK` / `READY`, generation-bound `TERMINAL_OUTPUT`,
and `BOOTSTRAP_TOMBSTONE`. What differs is the codec and the profile.

**Profile.** Profile negotiation is per stream. An `ATTACH_TERMINAL` to an
...
stamps its records consecutively and broadcasts them in one `TERMINAL_OUTPUT`
...
two calls never interleave. Live `TERMINAL_OUTPUT.bytes` carry one or more
...
Command_L1 = tagged_union {
    ...
    APPEND_RESOURCE_OUTPUT { terminal_id: TerminalId, bytes: bytes }, // tag 0x1a (§5.5)
}
...
<!-- impl-status: partial; probe: COMMAND_TAG_APPEND_RESOURCE_OUTPUT,NotProducer,RecordInvalid -->
> **Status: partial.** Tag `0x1a` and the error codes named below are in the
> reference codec, but no server handles the command. It is the input
> channel of the `AGENT_SESSION` kind (§1.1) and is served with it.

```
APPEND_RESOURCE_OUTPUT { resource_id: TerminalId, bytes: bytes }   // tag 0x1a
```
...
broadcast to the resource's subscribers in one `TERMINAL_OUTPUT` frame.
...
| 0x03 | `bell`             | (empty)                    |
| 0x04 | `pane_spawned`     | field-tagged TLV: `kind: optional<ResourceKind>` (field 1; absent = `TERMINAL`), `parent: optional<TerminalId>` (field 2; absent = root); empty for a root Terminal, id on envelope (§1.1, §1.2) |
...
- `pane_spawned` / `pane_closed` — sourced from the Terminal lifecycle.
  ...
  only those a client asked for by name: a `SPAWN_TERMINAL` adding a pane to
  ...
  `KILL_TERMINAL` path that emits `TERMINAL_CLOSED` (§3.1), and its
  `exit_status` carries the same value as `TERMINAL_CLOSED.exit_status`.
...
<!-- impl-status: partial; probe: AgentEventsJsonlV1 -->
> **Status: partial.** The reference detector ranks hook, process, and
> screen evidence ([ADR-0046](../../ADR/0046-server-side-agent-state-detection.md),
> [ADR-0085](../../ADR/0085-hook-sourced-agent-state.md)); the codec names
> the stream, but no server serves one to rank above them until the
> `AGENT_SESSION` kind lands.
...
- **Return leg.** Responses and subscribed stream frames arriving from the
  ...
  `BOOTSTRAP_TOMBSTONE`, `TERMINAL_OUTPUT`, `TERMINAL_CLOSED`, `BELL`) are
...
- **Two-hop attach.** The hub relays `ATTACH_TERMINAL` atomically with proxy
  ...
  the first `ATTACH_TERMINAL` still retires automatic spawn content before taking
  its fresh cut. Existing `SUBSCRIBE_TERMINAL_EVENTS` subscriptions and event
