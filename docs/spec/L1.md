...
TERMINAL_RESIZE  { terminal_id: TerminalId,
                   cols: u16,
                   rows: u16,
                   pixel_width: optional<u16>,
                   pixel_height: optional<u16> }
...
`pixel_width` (field id 4) and `pixel_height` (field id 5) optionally report
the target Terminal's total content-area pixel geometry. They MUST either both
be present and non-zero or both be absent. The server derives per-cell metrics
from `pixel / cells`, floored, and applies them only to the named Terminal's
PTY; an incomplete or sub-cell report does not replace established metrics.
These fields are distinct from the outer-client metrics on `VIEWPORT_RESIZE`
and let native clients report independently sized panes without allowing one
pane's geometry to resize another.
