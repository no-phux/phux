...
[`L3.md`](../spec/L3.md) §3.7) a server-side writer, so `state` is live.
...
[`L1.md`](../spec/L1.md) §6.2.1) shipped acknowledged with no CLI caller.
...
2. **`OK` is a kernel-queue receipt, and each typed error has one reading.** It
   ...
   and strictly less than consumption ([`L1.md`](../spec/L1.md) §6.2.1).
