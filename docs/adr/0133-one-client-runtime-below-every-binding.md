...
- The C ABI carries two lanes: `phux_client_new`'s embedded one, where the
  embedder owns the socket, and `phux_client_connect`'s connected one. The
  embedded lane stays because a harness feeds synthetic frames through it;
  the cost is that the two must be kept mutually exclusive at runtime.
